...
def instance_manager(instance_id):
    role_priority = ['Tank', 'Healer', 'DPS']  # Initialize role_priority inside the function

    while True: 
        instances_sem.acquire()

        # Check if all queues are empty, then break the loop
        if tank_q.empty() and healer_q.empty() and dps_q.empty():
            instances_sem.release()
            break

        # Rotate roles to avoid starvation
        role_priority = role_priority[1:] + role_priority[:1]

       # Check if there are enough players of each role to form a party
        if not (tank_q.qsize() >= 1 and healer_q.qsize() >= 1 and dps_q.qsize() >= 3):
            instances_sem.release()
            continue  # Continue to the next iteration to check again

        # Forming a party based on role priority
        if role_priority[0] == 'Tank' and tank_q.qsize() >= 1:
            tank = tank_q.get()
        elif role_priority[0] == 'Healer' and healer_q.qsize() >= 1:
            healer = healer_q.get()
        elif dps_q.qsize() >= 3:
